...
export default function Table() {
  ...
  const getTableAction = (action: {
    ...
    getData({
      page:
        action.pagination.current == pagination.page
          ? 1
          : action.pagination.current,
      limit: action.pagination.pageSize,
      sortBy: action.sorter?.columnKey,
      sortOrder: action.sorter?.order
        ? action.sorter?.order == "ascend"
          ? "ASC"
          : "DESC"
        : undefined,
    });
  ...
  const actions = [
    {
      ...
      actionHandler: (content: any) => {
        Modal.confirm({
          ...
          cancelText: "No",
          onOk() {
            setBackData((prevData) =>
              prevData.filter((row) => row.id !== content.id)
            );
          },
          onCancel() {
            console.log("Cancel");
    ...
    },
    {
      ...
      permission: "callingapi_jobs:update",
    },
  ];
  ...
  return (
    <div>
      <DataTable
        permissoinName={`callingapi_jobs:list`}
        tableTitle="Users"
        columns={Backcolumns}
        actions={actions}
        data={backData}
        pagination={pagination}
        onTableAction={getTableAction}
        loading={loading}
